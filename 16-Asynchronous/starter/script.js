...
const renderError = function (msg) {
  const html = `there is sometthing wrong. ${msg}.`;
  countriesContainer.insertAdjacentHTML('beforeend', html);
  countriesContainer.style.opacity =1;
};
...
//             <h3 class="country__name">${data.name.common}</h3>
//             <h4 class="country__region">${data.region}</h4>
//             <p class="country__row"><span>👫</span>${Number(data.population/100000).toFixed(1)} people</p>
...
// }
// const ctr1 = getCountryAndNeighbour("turkey")
...
//         if (!response.ok)
//             throw new Error(`Country coudnt find. (${response.status})`);
...
//         console.log(response);
//         if (!response.ok)
//             throw new Error(`Neighbour Country coudnt find. (${response.status})`);
...
const getJSON = function (url, errMsg = 'Something went go') {
  return fetch(url).then(response => {
    console.log(response);
    if (!response.ok)
      throw new Error(`${errMsg} and status :  (${response.status})`);
...
const getCountryData = function (country) {
  getJSON(
    ...
    .then(data => {
      ...
      console.log(data[0]);
      if (!data[0].borders)
        throw new Error('there is no any neigbours for this country');
      const [neighbour] = data[0].borders;
      //neighbour ="dfsfsffd"

      return getJSON(
        `https://restcountries.com/v3.1/name/${neighbour}`,
        'Neighbour Country coudnt find'
      );
    })
    .then(data => renderCountry(data[0], 'neighbour'))
    .catch(err => {
...
a city and country name. Use this API to do reverse geocoding: https://geocode.xyz/api.
The AJAX call will be done to a URL with this format: https://geocode.xyz/52.508,13.381?geoit=json. Use the fetch API and promises to 
...
5. This API allows you to make only 3 requests per second. If you reload fast, you will get this error with code 403. This is an error 
with the request. Remember, fetch() does NOT reject the promise in this case. So create an error to reject the promise yourself, with a meaningful error message.
...
//       //console.log(response);
//       if (!response.ok)
//         throw new Error(`Geocoding web site problem  ${response.status}`);
//       return response.json();
//     })

//     .then(data => {
//       if (!data) throw new Error('Country is not found');
...
to the provided image path. When the image is done loading, 
append it to the DOM element with the 'images' class, 
and resolve the promise. The fulfilled value should be 
...
(HINT: Use the image element returned by the createImage 
  promise to hide the current image. You will need a global 
  variable for that 😉);
...
// const loadImgPause = async function () {
...
const resGeoCode = await fetch(`https://geocode.xyz/${lat},${lng}?geoit=json`);
...
console.log(data)
renderCountry(data[0])} catch(err) {console.error(first)}
